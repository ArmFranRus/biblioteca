...
export const api = createApi({
  ...
  endpoints: (builder) => ({
    ...
    addCopy: builder.mutation<
      Copia,
      { libroId: number; codiceInventario: string }
    >({
    ...
    updateCopyState: builder.mutation<
      Copia,
      { id: number; libroId: number; stato: StatoCopia }
    >({
      query: ({ id, stato }) => ({
        url: `copies/update/${id}`,
        method: "PATCH",
        body: { stato },
      }),
      invalidatesTags: (_res, _err, { libroId }) => [
    ...
    getUsers: builder.query<Utente[], void>({
      query: () => "users/all",
      providesTags: ["Users"],
    }),
    ...
    setUserEnabled: builder.mutation<Utente, { id: number; enabled: boolean }>({
      query: ({ id, enabled }) => ({
        url: `users/${enabled ? "enable" : "disable"}/${id}`,
        method: "PATCH",
      }),
      invalidatesTags: ["Users"],
    }),
    ...
    getLoans: builder.query<
      Prestito[],
      { utenteId?: number; stato?: StatoPrestito } | void
    >({
    ...
    createLoan: builder.mutation<
      Prestito,
      { copiaId: number; utenteId: number; libroId: number }
    >({
    ...
    createReview: builder.mutation<
      Recensione,
      { libroId: number; voto: number; testo: string }
    >({
